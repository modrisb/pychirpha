...
from pychirpha.const import ERRMSG_CODEC_ERROR, ERRMSG_DEVICE_IGNORED, WARMSG_DISC_AUTO
...
@mock.patch("pychirpha.grpc.dukpy", new=dukpy)
...
def test_codec_prologue_issues(caplog):
    ...
    def run_test_codec_prologue_issues(config):
        assert mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).stat_devices == 1  # generated codec used, device count = 1
        assert mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).stat_sensors == 2  # generated codec used, sensor count = 2
