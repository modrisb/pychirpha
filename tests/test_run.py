...
from pychirpha.const import BRIDGE_CONF_COUNT, CONF_APPLICATION_ID
from pychirpha.const import CONF_OPTIONS_START_DELAY, CONF_OPTIONS_RESTORE_AGE, CONF_OPTIONS_ONLINE_PER_DEVICE, CONF_MQTT_CHIRPSTACK_PREFIX
...
def test_device_status_refresh(caplog):
    ...
    def run_test_device_status_refresh(config):
        ...
        time.sleep(MIN_SLEEP+MIN_SLEEP+3)
        for i in range(0, mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).stat_devices):
            dev_eui = f"dev_eui{i}"
            topic = f"application/{config.get(CONF_APPLICATION_ID)}/device/{dev_eui}/event/cur"
        ...
        no_of_up_msgs = common.count_messages(r'/device/.*/up$', None, keep_history=True)    # should not show up due to time stamp
        config_topics = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).get_published()
        assert no_of_cur_msgs == 4
        assert no_of_up_msgs == 0
...
def test_init_live_overlapping(caplog):
    ...
    def run_test_init_live_overlapping(config):
        ...
        online_period = config.get(CONF_OPTIONS_ONLINE_PER_DEVICE) * 60 # to seconds
        time.sleep(MIN_SLEEP+1.7) # this force windows to overlap
        topic = f"application/{config.get(CONF_APPLICATION_ID)}/bridge/live"
        msg = "start"
        mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).publish(topic, msg)
        mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).wait_empty_queue()
        time.sleep(restore_age+online_period+MIN_SLEEP)
...
def test_chirpstack_prefix(caplog):
    """Test payload join for array data with more than 1 sublevel."""

    def run_test_chirpstack_prefix(config):
        config_topics = mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).get_published()
        mqtt.Client(mqtt.CallbackAPIVersion.VERSION2).wait_empty_queue()
        app_counter_with_prefix = 0
        app_counter_with_no_prefix = 0
        for config_topic in config_topics:
            if config_topic[0].startswith(test_vals[1]):
                app_counter_with_prefix += 1
            elif "application/" in config_topic[0]:
                app_counter_with_no_prefix += 1
        assert app_counter_with_prefix > 0
        assert app_counter_with_no_prefix <= 0

    for test_vals in (("", "application/"), ("test", "test/application/"), ("test/", "test/application/")):
        common.chirp_setup_and_run_test(caplog, run_test_chirpstack_prefix, test_params=dict(devices=1),
                                        conf_file=REGULAR_CONFIGURATION_FILE, allowed_msg_level=logging.WARNING,
                                        extra_conf={CONF_MQTT_CHIRPSTACK_PREFIX:test_vals[0]})
