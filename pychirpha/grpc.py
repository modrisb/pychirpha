...
class ChirpGrpc:
    ...
    def __init__(self, config, version) -> None:
        ...
        self.application_id = self._config.get(CONF_APPLICATION_ID)
        ...
        _LOGGER.info(
            "The gRPC channel opened for %s:%s, token id %s",
            self._config.get(CONF_API_SERVER),
            self._config.get(CONF_API_PORT),
            self._token_id,
        )
        self.gateway_id = None
        tenant_id = self.is_valid_app_id(self.application_id)
        if tenant_id is None:
            ...
            if len(tenants_on_chirp) == 0:
                ...
                _LOGGER.info(
                    "Tenant '%s' (id %s) created",
                    createTenantReq.tenant.name,
                    tenantResp.id,
                )
                tenants_on_chirp = self.get_chirp_tenants()
            tenant_name = None
            ...
            if len(applications_on_chirp) == 0:
                ...
                applicationResp = application.Create(
                    createApplicationReq, metadata=self._auth_token
                )
                _LOGGER.info(
                    "Application '%s' (id %s, tenant %s) created",
                    createApplicationReq.application.name,
                    applicationResp.id,
                    tenant_name,
                )
                applications_on_chirp = self.get_tenant_applications(tenant_id)
            ...
            application_id = None
            for application_name, application_id in applications_on_chirp.items():  # noqa: B007
                break
        ...
        _LOGGER.info("ChirpStack server gateway IDs %s", self.gateway_id)

    def __del__(self):
        """Close grpc channel on exit."""
        self.close()

    def get_chirp_tenants(self) -> dict[str, str]:
    ...
    def get_tenant_applications(self, tenant_id) -> dict[str, str]:
        ...
        listApplicationsReq = api.ListApplicationsRequest(tenant_id=tenant_id)
    ...
    def is_valid_app_id(self, application_id) -> str | None:
        """Check application id validity with api server."""
        application = api.ApplicationServiceStub(self._channel)
        applicationReq = api.GetApplicationRequest(id=application_id)
        try:
            application_result = application.Get(
                applicationReq, metadata=self._auth_token
            )
        except Exception:  # noqa: BLE001
            return None
        return application_result.application.tenant_id
    ...
    def get_chirp_app_devices(self):
        ...
        listDevicesReq.application_id = self.application_id
    ...
    def get_chirp_device(self, dev_eui):
        ...
        deviceReq = api.GetDeviceRequest(dev_eui=dev_eui)
        return device.Get(deviceReq, metadata=self._auth_token)
    ...
    def get_chirp_device_profile(self, device_profile_id):
        ...
        listDevicesReq = api.GetDeviceProfileRequest(id=device_profile_id)
    ...
    def get_chirp_gateway_ids(self):
        """Get chirp server gateway ids."""
        gateway = api.GatewayServiceStub(self._channel)
        listGatewaysReq = api.ListGatewaysRequest()
        gatewaysResp = gateway.List(listGatewaysReq, metadata=self._auth_token)
        listGatewaysReq.limit = gatewaysResp.total_count
        return gateway.List(listGatewaysReq, metadata=self._auth_token).result
    ...
    def close(self):
        ...
        if self._channel:
            self._channel.close()
            self._channel = None
        _LOGGER.debug("gRPC close complete")
        ...
        for device in devices:
            ...
            try:
                ...
                if not mi_start:
                    ...
                    codec_code = generate_getHaDeviceInfo(
                        profile.device_profile.payload_codec_script,
                        profile.device_profile.description,
                        profile.device_profile.name,
                        not device.device_status.external_power_source,
                ...
                if mi_start:
                    ...
                    discovery = self.js_interpreter.evaljs(
                        codec_code + "; JSON.stringify(getHaDeviceInfo())"
                    )
            ...
            for entity, config in discovery["entities"].items():
                ...
                if not discovery_config.get("value_template"):
                    discovery_config["value_template"] = (
                        f"{{{{ value_json.object.{entity} }}}}"
                    )
                discovery_config["uplink_interval"] = (
                    profile.device_profile.uplink_interval
                )
    ...
    def get_device_visibility_info(self, dev_eui):
        ...
        visibility["device_status_req_interval"] = (
            profile.device_profile.device_status_req_interval
        )
        visibility["last_seen"] = (
            device.last_seen_at.seconds + device.last_seen_at.nanos * 1e-9
            if str(device.last_seen_at)
            else 0
        )
        return visibility
